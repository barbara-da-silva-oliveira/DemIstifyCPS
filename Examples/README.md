...
## 2. Core concepts in the `.inf` file

Within this model, the main concepts are:
...
- **Definition:** A named handle referencing an element in the SysML models (here, the wheel controller’s `speedWheel` parameter).
- **`changeability`:** How easy it is organizationally or technically to change this artifact. Higher value = easier / cheaper change.

Other artifacts in the model:

- `motorSpeed`
- `detectionThreshold`
- `delayStaticDetection`
- `sweepingDelay`
- `computeDistance`
- `ultrasonicSensor`
- `motor`
...
- **Definition:** A named environment parameter.
- **`description`:** Human-readable meaning.
- **`flexibility` [0–1]:** How much this factor can realistically be changed (e.g., by choosing other test conditions, surfaces, etc.).
- **`value`:** Default numeric value used in analyses.
...
### 2.3 SystemResponseProperty (SRP)
...
- **Definition:** A named, observable outcome of the system (often a quantity derived from physics or logs).
- Some SRPs are constrained by requirements:
...
Requirement EfficientExploration
  ...
  satisfied when "The robot explores the environment efficiently, avoiding unnecessary maneuvers"
  languageType NaturalLanguage,
```
...
- **`description`:** Informal statement.
- **`satisfied when`:** Either:
  - informal text (`NaturalLanguage`), or
  - a numeric constraint (`JavaNumericExpression`) that can be evaluated (here: `MarginCollision - 0.05 > 0`).

### 2.5 Influence

Each `Influence` connects:

- a set of **participants** (artifacts, environmental factors, and/or upstream SRPs),
- an **influenceFunction** (textual or numeric), and
- one **output SRP** (via `affects`).
...
- **Participants:** Each participant has a `confidence` describing how sure you are that this element really participates in the influence.
- **Influence function:** Can be a numeric expression (`Real` + `JavaNumericExpression`) or an informal `Text` description (`NaturalLanguage`).
- **Output SRP:** Exactly one SRP per influence (`affects DistanceToStop`).
