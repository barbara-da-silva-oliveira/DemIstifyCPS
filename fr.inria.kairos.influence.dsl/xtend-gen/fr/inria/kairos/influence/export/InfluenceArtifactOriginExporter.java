...
public class InfluenceArtifactOriginExporter {
  ...
  public List<InfluenceArtifactOriginExporter.Row> analyze(final Resource res) {
    ArrayList<InfluenceArtifactOriginExporter.Row> _xblockexpression = null;
    {
      final ArrayList<InfluenceArtifactOriginExporter.Row> rows = new ArrayList<InfluenceArtifactOriginExporter.Row>();
      ...
      for (final Influence inf : _filter) {
        {
          ...
          if (_isEmpty) {
            InfluenceArtifactOriginExporter.Row _row = new InfluenceArtifactOriginExporter.Row(iName, 0, "");
            rows.add(_row);
          } else {
            ...
            InfluenceArtifactOriginExporter.Row _row_1 = new InfluenceArtifactOriginExporter.Row(iName, _size, _joinSemi);
            rows.add(_row_1);
          }
        }
      }
  ...
  public void exportCSV(final Resource res, final IFileSystemAccess2 fsa, final String outPath) {
    final List<InfluenceArtifactOriginExporter.Row> rows = this.analyze(res);
    final StringBuilder sb = new StringBuilder("influence,model_count,models\n");
    for (final InfluenceArtifactOriginExporter.Row r : rows) {
