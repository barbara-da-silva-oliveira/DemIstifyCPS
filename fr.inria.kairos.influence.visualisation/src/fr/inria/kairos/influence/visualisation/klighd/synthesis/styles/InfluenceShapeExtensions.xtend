 ...
 */
@ViewSynthesisShared
class InfluenceShapeExtensions extends AbstractSynthesisExtensions {
    ...
    public static val REACTOR_CONTENT_CONTAINER = new Property<Boolean>("fr.inria.kairos.influence.visualisation.klighd.synthesis.shapes.reactor.content", false)
...
//	@Inject extension KNodeExtensions
	 ...
	 */
	def addCompositeInfluenceFigure(KNode node, CompositeInfluence influence, String text) {
		val padding = SHOW_HYPERLINKS.booleanValue ? 8 : 6
		val figure = node.addRoundedRectangle(8, 8, 1) => [
			setGridPlacement(1)
			lineWidth = 1
			foreground = Colors.RED_3
	 ...
	 */
	def addInfluenceFigure(KNode node, Influence influence, String text) {
		val padding = SHOW_HYPERLINKS.booleanValue ? 8 : 6
		val figure = node.addRoundedRectangle(8, 8, 1) => [
			setGridPlacement(1)
			lineWidth = 1
			foreground = Colors.BURLYWOOD_4
			boldLineSelectionStyle
		]
	 ...
	 * Creates the artefact frame.
	 */
	def addDesignArtifactFigure(KNode node, DesignArtifact artifact, String text) {
		val padding = SHOW_HYPERLINKS.booleanValue ? 8 : 6
		val figure = node.addRoundedRectangle(8, 8, 1) => [
			setGridPlacement(1)
			lineWidth = 1
...
//	 */
//	def ReactorFigureComponents addReactorFigure(KNode node, FInterface reactor, FDExtensionElement instance, String text) {
//		node.associateWith(instance)
//		
//		val padding = SHOW_HYPERLINKS.booleanValue ? 8 : 6
...
//		figure.addRectangle() => [
//			invisible = true
//			setGridPlacementData().from(LEFT, padding, 0, TOP, padding, 0).to(RIGHT, padding, 0, BOTTOM, reactor.contract !== null ? 4 : padding, 0)
//			
//			addRectangle() => [ // Centered child container
//				invisible = true
//				setPointPlacementData(LEFT, 0, 0.5f, TOP, 0, 0.5f, H_CENTRAL, V_TOP, 0, 0, 0, 0)
//				val placement = setGridPlacement(1)
//				var isIpDefined = false
//				var ipValue = ""
//				for (prop : instance.properties.items){
//					if (prop.decl.name == "IPAddress"){
//						isIpDefined = true
//						ipValue= (prop.value.single as FDString).value
//					}
//					if (prop.decl.name == "Port"){
//						ipValue += ':'+(prop.value.single as FDInteger).value
//					}
//					placement.numColumns = 2
//				}
//				
//				if (instance !== null && isIpDefined) {
//					addCloudUploadIcon() => [
//						setGridPlacementData().from(LEFT, 10, 0, TOP, 0, 0).to(RIGHT, 10, 0, BOTTOM, 0, 0)
//					]
//				}
//				addText(text) => [
//					suppressSelectability
